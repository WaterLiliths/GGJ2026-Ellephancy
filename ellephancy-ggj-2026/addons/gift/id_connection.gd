...
func get_user_info(parent: Node) -> Dictionary:
	var http := HTTPRequest.new()
	parent.add_child(http)

	var headers := [
		"Authorization: Bearer %s" % last_token.token,
		"Client-Id: %s" % last_token.last_client_id
	]

	var err := http.request("https://api.twitch.tv/helix/users", headers)
	if err != OK:
		push_error("Falló al iniciar la request: %s" % err)
		return {}
	...
	if result[0] != HTTPRequest.RESULT_SUCCESS:
		push_error("Request fallida: código %s" % result[0])
		return {}

	var json := JSON.parse_string(body.get_string_from_utf8())
	if typeof(json) == TYPE_DICTIONARY and json.has("data") and json["data"].size() > 0:
		return json["data"][0]
	else:
		push_error("Respuesta inválida: %s" % body.get_string_from_utf8())
		return {}
